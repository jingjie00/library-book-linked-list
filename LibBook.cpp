...
void LibBook::print(ostream& out)
{
	out << "\nTitle: " << title;
	out << "\nAuthor: ";
	for (int i = 0; author[i] != NULL; i++) //author can only be 9 people because the termination of the for loop is detect null. Thus, we need to left out one space for it.To prevent overflow.
		out << author[i] << '\t';
	out << "\nPublisher: " << publisher;
